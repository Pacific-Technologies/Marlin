...
public static class UpdateBestAskAggregateHandler
{
    public static Events Handle(UpdateBestAsk command, OrderBook state, ILogger logger)
    {
        try
        {
        ...
        {
            logger.LogError(ex, $"Error executing command: {nameof(UpdateBestAsk)}");
            throw;
        }
    }
}